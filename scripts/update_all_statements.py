...
from google_sheets.statements.all_statements_tab import AllStatementsTab
...
def statements_tab_for_month(month: AccountingMonth) -> AllStatementsTab:
    id = AllStatementsTab.sheet_id_for_month(month)
    return AllStatementsTab(
        Workbook(id),
        month.month_name,
        month
    )
...
def statements_consistent(tab: AllStatementsTab, activity: BankActivity, fail_on_inconsistency: bool) -> bool:
...
def ensure_tab_consistent(month: AccountingMonth, refresh_bank_activity: bool, refresh_sheet: bool):
    ...
    current_account_sheet_id = StatementsTab.sheet_id_for_account(CURRENT_ACCOUNT, month)
    old_current_account_tab = StatementsTab(Workbook(current_account_sheet_id), CURRENT_ACCOUNT, month.month_name, month)
...
def compare_uncategorized_with_kashflow(month: AccountingMonth):
    sheet_id = AllStatementsTab.sheet_id_for_month(month)
    tab = AllStatementsTab(Workbook(sheet_id), month.month_name, month)
