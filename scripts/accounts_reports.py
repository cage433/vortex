...
def create_accounting_tab(periods: List[DateRange], period_names: List[str], title, show_transactions):
    ...
    bank_activity = read_bank_activity(bounding_period, force=False)
    tab = AccountingReportTab(workbook, title,
                              periods, period_names, gigs_info, nominal_ledger, bank_activity, show_transactions)
    tab.update()


def create_month_tab(month: AccountingMonth):
    weeks = month.weeks
    period_titles = [f"W{w.week_no}" for w in weeks]
    title = f"MTD {month.month_name}"
    create_accounting_tab(weeks, period_titles, title, show_transactions=True)


def create_ytd_tab(year: AccountingYear, show_transactions: bool):
    ...
    title = f"YTD {year.y}"
    create_accounting_tab(months, period_titles, title, show_transactions=show_transactions)
